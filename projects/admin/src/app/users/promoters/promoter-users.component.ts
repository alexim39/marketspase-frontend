...
@Component({
  ...
  templateUrl: './promoter-users.component.html',
  styleUrls: ['./promoter-users.component.scss'],
})
export class PromoterUserMgtComponent implements OnInit, OnDestroy {
  ...
  private loadRoleStats(): void {
    ...
    this.userService.getStatsByRole('promoter').pipe(takeUntil(this.destroy$)).subscribe({
      next: (response) => {
        if (response.success) this.roleStats.set(response.data as any);
  ...
  loadPromoters(): void {
    ...
    this.userService.getUsersByRole('promoter', filters).pipe(takeUntil(this.destroy$)).subscribe({
      next: (response) => {
        if (response.success) {
      ...
      error: () => {
        this.isLoading.set(false);
        this.showError('Failed to load promoters');
      }
    });
  }
  ...
  activateUser(user: UserInterface): void {
    this.userService.updateUserStatus(user._id, true).pipe(takeUntil(this.destroy$)).subscribe({
      next: () => { this.showSuccess('User activated'); this.loadPromoters(); this.loadRoleStats(); },
  ...
  deactivateUser(user: UserInterface): void {
    this.userService.updateUserStatus(user._id, false).pipe(takeUntil(this.destroy$)).subscribe({
      next: () => { this.showSuccess('User deactivated'); this.loadPromoters(); this.loadRoleStats(); },

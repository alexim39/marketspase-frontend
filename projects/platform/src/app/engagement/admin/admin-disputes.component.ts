...
@Component({
          ...
          @for (d of disputes(); track d._id) {
            <mat-card class="dispute-card">
              <mat-card-content>
                ...
                <div class="parties">
                  <div class="party">
                    <span class="role">Marketer</span>
                    <strong>{{ d.marketerId?.displayName }}</strong>
                    <small>{{ d.marketerId?.email }}</small>
                  </div>
                  <mat-icon>swap_horiz</mat-icon>
                  <div class="party" style="text-align:right">
                    <span class="role">Promoter</span>
                    <strong>{{ d.promoterId?.displayName }}</strong>
                    <small>{{ d.promoterId?.email }}</small>
                  </div>
                </div>
                <div class="tasks-row">
  ...
  styles: [`
    ...
    .count-badge { padding: 4px 12px; border-radius: 999px; font-size: 0.8rem; font-weight: 700; background: rgba(var(--error-rgb), 0.1); color: var(--error-color); }
    .disputes-list { display: flex; flex-direction: column; gap: 14px; }
    .dispute-card { border-left: 3px solid var(--error-color); }
    .dispute-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px; .status-disputed { display: inline-block; padding: 2px 8px; border-radius: 999px; font-size: 0.65rem; font-weight: 700; background: rgba(var(--error-rgb), 0.1); color: var(--error-color); } .date { font-size: 0.7rem; color: var(--text-tertiary); display: block; } .amount { font-weight: 800; color: var(--primary-color); font-size: 1.1rem; } }
    ...
    .dispute-reason { margin-bottom: 14px; padding: 10px 12px; background: rgba(var(--error-rgb), 0.04); border-radius: 8px; strong { font-size: 0.75rem; color: var(--error-color); } p { margin: 4px 0 0; font-size: 0.82rem; color: var(--text-secondary); } }
...
export class AdminDisputesComponent implements OnInit {
  ...
  resolve(contractId: string, resolution: 'refund-marketer' | 'release-promoter'): void {
    this.api.post<any>(`api/v1/social/admin/disputes/${contractId}/resolve`, { resolution }, undefined, true).subscribe({
      next: () => {
        this.snack.open(`Dispute resolved: ${resolution === 'refund-marketer' ? 'Refunded marketer' : 'Released to promoter'}`, 'OK', { duration: 3000 });
        this.loadDisputes();
      },
